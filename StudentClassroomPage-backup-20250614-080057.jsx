...
import "../../assets/css/RatingModal.style.css";
...
const StudentClassroomPage = () => {
  ...
  // Rating state for meeting evaluation
  ...
  const fetchStudentClassrooms = useCallback(
    async (page /* statusFilter = null */) => {
      ...
      try {
        console.log(
          "🔍 Fetching all student classrooms using token authentication only"
        );
        console.log(
          "📊 Note: API returns ALL classrooms, client-side filtering applied for status tabs"
        );
    ...
    if (view === "detail" && classroomId && classroomName) {
      ...
      const restoreMeetingView = async () => {
        try {
          ...
          console.log(
            "🔍 STUDENT DEBUG - Restoring meeting view, trying meeting/get-meeting first"
          );
          ...
          console.log(
            "🔍 STUDENT DEBUG - meeting/get-meeting restore response:",
            response
          );
          console.log(
            "🔍 STUDENT DEBUG - Full restore response structure:",
          ...
          if (response.success) {
            if (
              ...
              console.log(
                "✅ STUDENT DEBUG - Restored meetings via meeting/get-meeting from data.result.items:",
                allMeetingsData.length
              );
            } else if (response.result && response.result.items) {
              allMeetingsData = response.result.items;
              console.log(
                "✅ STUDENT DEBUG - Restored meetings via meeting/get-meeting from result.items (fallback):",
                allMeetingsData.length
              );
            } else if (response.data && response.data.items) {
              allMeetingsData = response.data.items;
              console.log(
                "✅ STUDENT DEBUG - Restored meetings via meeting/get-meeting from data.items (fallback 2):",
                allMeetingsData.length
              );
            } else {
              console.log(
                "⚠️ STUDENT DEBUG - No meetings found in response.data.result.items, response.result.items, or response.data.items during restore"
              );
            }
          ...
          } // If we reach here, no meetings found
          console.log(
            "⚠️ STUDENT DEBUG - No meetings found, showing empty state"
          );
          setMeetingList([]);
          setTotalMeetings(0);
          setShowMeetingView(true);
  ...
  const handleViewMeetings = async (
    ...
    // CRITICAL: Tìm full classroom data từ allClassrooms nếu classroom param thiếu data
    ...
    try {
      ...
      console.log("🔍 STUDENT DEBUG - meeting/get-meeting response:", response);
      console.log(
      ...
      if (response.success) {
        ...
        if (allMeetingsData.length > 0) {
          ...
          // CRITICAL DEBUG: Check what happens with student filter approach
          ...
          setMeetingList(allMeetingsData);
          setTotalMeetings(allMeetingsData.length);
          setCurrentMeetingPage(page);          // ✅ FIX: Truyền đầy đủ classroom data cho rating
          setCurrentClassroomForMeetings({
            classroomId,
            ...
            // ✅ FIX: Đảm bảo tutor data được truyền đúng
            tutor: fullClassroomData?.tutor || null,
            tutorId: fullClassroomData?.tutorId || fullClassroomData?.tutor?.userId || null,
          ...
          // Set URL params like TutorClassroomPage for consistency
          setSearchParams({
            view: "meetings",
      ...
      } // If we reach here, meeting/get-meeting returned no data
      console.log("⚠️ STUDENT DEBUG - No meetings found, showing empty state");
      setMeetingList([]);
      setTotalMeetings(0);
      setCurrentMeetingPage(page);      // ✅ FIX: Truyền đầy đủ classroom data cho rating (empty case)
      setCurrentClassroomForMeetings({
        classroomId,
        nameOfRoom: classroomName,
        // ✅ THÊM: Data cần thiết cho rating
      ...
      console.error("❌ STUDENT DEBUG - Error fetching meetings:", error);
      toast.error("Không thể tải danh sách phòng học.");
      setMeetingList([]);
      setTotalMeetings(0); // Still show meeting view even on error
      setCurrentMeetingPage(page);      // ✅ FIX: Truyền đầy đủ classroom data cho rating (error case)
      setCurrentClassroomForMeetings({
        classroomId,
        nameOfRoom: classroomName,
        // ✅ THÊM: Data cần thiết cho rating
  ...
  // Rating Functions  const handleOpenRatingModal = (meeting, classroom) => {
    console.log("🔍 RATING DEBUG - Opening rating modal:", {
      meetingId: meeting?.meetingId,
      classroomId: classroom?.classroomId,
      tutorId: classroom?.tutor?.userId || classroom?.tutorId,
      tutorFromObject: classroom?.tutor?.userId,
      tutorFromField: classroom?.tutorId,
      isRating: classroom?.isRating,
      hasAllData: !!(meeting && classroom && (classroom.tutor || classroom.tutorId)),
    });

    if (!meeting) {
      console.error("❌ RATING ERROR - Missing meeting data");
      toast.error("Thiếu thông tin meeting để đánh giá!");
      return;
    }

    if (!classroom) {
      console.error("❌ RATING ERROR - Missing classroom data");
      toast.error("Thiếu thông tin classroom để đánh giá!");
      return;
    }

    // ✅ FIX: Kiểm tra tutorId từ cả tutor object và tutorId field
    const tutorId = classroom.tutor?.userId || classroom.tutorId;
    if (!tutorId) {
      console.error("❌ RATING ERROR - Missing tutor data", {
        tutorObject: classroom.tutor,
        tutorIdField: classroom.tutorId,
        fullClassroom: classroom
      });
      toast.error("Thiếu thông tin gia sư để đánh giá!");
      return;
    }

    console.log("✅ RATING DEBUG - Setting modal state...");
    setSelectedMeetingForRating(meeting);
    setCurrentClassroomForRating(classroom);
    setRatingValue(0);
    setRatingDescription("");
    setShowRatingModal(true);

    console.log(
      "✅ RATING DEBUG - Modal state set, showRatingModal should be true"
    );
  };
  ...
  const handleCloseRatingModal = () => {
    ...
    setCurrentClassroomForRating(null);
    setRatingValue(0);
    setRatingDescription("");
  };
  ...
  const handleSubmitRating = async () => {
    if (!selectedMeetingForRating || !currentClassroomForRating) {
    ...
    if (!ratingDescription.trim()) {
      toast.error("Vui lòng nhập mô tả đánh giá!");
      return;
    }

    try {
      setIsSubmittingRating(true);      const ratingData = {
        tutorId: currentClassroomForRating.tutor?.userId || currentClassroomForRating.tutorId,
        classroomEvaluation: ratingValue,
        description: ratingDescription.trim(),
        meetingId: selectedMeetingForRating.meetingId,
      };

      console.log("🔍 RATING DEBUG - Submitting rating:", ratingData);
      ...
      console.log("🔍 RATING DEBUG - Rating response:", response);

      if (response.success) {
        toast.success("Đánh giá đã được gửi thành công!");
        handleCloseRatingModal();

        // Refresh classroom list to update isRating flag
        console.log("🔍 RATING DEBUG - Refreshing classroom list...");
        fetchStudentClassrooms(currentPage, true); // Force refresh
      } else {
        toast.error(
          response.message || "Không thể gửi đánh giá. Vui lòng thử lại!"
        );
      }
    } catch (error) {
      console.error("❌ RATING DEBUG - Error submitting rating:", error);
  ...
  const StarRating = ({
    ...
    for (let i = 1; i <= 5; i++) {
      ...
      stars.push(
        <span
          ...
          onClick={() => !readonly && onStarClick && onStarClick(i)}
          style={{
            fontSize: size,
    ...
    // Add half-star options for interactive rating
  ...
  // Props validation for StarRating
  ...
  const RatingModal = () => {
    console.log("🔍 RATING MODAL DEBUG - Render check:", {
      ...
      currentClassroomForRating: currentClassroomForRating?.classroomId,
      shouldShow:
        showRatingModal &&
    ...
    return (
      <div className="scp-modal-overlay" onClick={handleCloseRatingModal}>
        <div className="scp-modal-content" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="scp-modal-body">
            <div className="scp-rating-info">
              <p>
                <strong>Phòng học:</strong> {selectedMeetingForRating.topic}
              </p>
              <p>
                <strong>Gia sư:</strong>{" "}
                {currentClassroomForRating.tutor?.fullname}
              </p>
              <p>
                <strong>Meeting ID:</strong>{" "}
                {selectedMeetingForRating.meetingId}
              </p>
            </div>

            <div className="scp-rating-section">
              <label>Đánh giá số sao:</label>
              <div className="scp-star-rating">
                <StarRating
                  rating={ratingValue}
                ...
                <span className="scp-rating-value">
                  {ratingValue > 0 ? `${ratingValue} sao` : "Chưa chọn"}
                </span>
              </div>
            </div>

            <div className="scp-description-section">
              <label>Mô tả đánh giá:</label>
              <textarea
                value={ratingDescription}
                onChange={(e) => setRatingDescription(e.target.value)}
                placeholder="Nhập mô tả đánh giá về gia sư và phòng học..."
                rows="4"
                disabled={isSubmittingRating}
                maxLength={500}
              />
              <small>{ratingDescription.length}/500 ký tự</small>
            </div>
          </div>

          <div className="scp-modal-footer">
            <button
            ...
            <button
              ...
              onClick={() =>
                handleGoToMeetingView(
                  ...
                  classroom // ✅ PASS FULL CLASSROOM OBJECT
                )
              }
            >
                    ...
                    return (
                      <li key={index} className="scp-meeting-item">
                        ...
                        <div className="scp-meeting-actions">
                          ...
                          {/* Rating Section - Show for all meetings */}
                          <div className="scp-meeting-rating">
                            ...
                            currentClassroomForMeetings.isRating ? (
                              ...
                              <div className="scp-rating-display">
                                <span className="scp-rating-label">
                                  Đã đánh giá:
                                </span>
                                <StarRating
                                  rating={parseFloat(
                                    currentClassroomForMeetings.classroomEvaluation ||
                                      0
                                  )}
                                  readonly={true}
                                  size="18px"
                              ...
                              <button
                                ...
                                onClick={() => {
                                  console.log("🔍 RATING BUTTON CLICKED:", {
                                    ...
                                    isRating:
                                      currentClassroomForMeetings?.isRating,
                                    tutor: currentClassroomForMeetings?.tutor,
                                  });
                                  console.log(
                                    "🔍 CALLING handleOpenRatingModal with:",
                                    {
                                      meetingData: meeting,
                                      classroomData:
                                        currentClassroomForMeetings,
