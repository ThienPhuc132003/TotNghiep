...
const CreateMeetingTest = () => {
  ...
  const testCreateMeeting = async () => {
    ...
    // Check Zoom token first
    const zoomToken = localStorage.getItem("zoomAccessToken");
    if (!zoomToken) {
      addLog("❌ No Zoom Access Token found! Please connect your Zoom account first.", "error");
      addLog("💡 Go to Zoom settings page and connect your account", "info");
      return;
    }
    
    addLog("✅ Zoom Access Token found", "success");
  ...
  const testZoomToken = async () => {
    addLog("🔍 Checking Zoom Access Token...", "info");

    const zoomToken = localStorage.getItem("zoomAccessToken");
    if (!zoomToken) {
      addLog("❌ No Zoom Access Token found in localStorage", "error");
      addLog("💡 You need to connect your Zoom account first", "info");
      return;
    }

    addLog("✅ Zoom Access Token found", "success");
    addLog(`🔑 Token: ${zoomToken.substring(0, 20)}...`, "debug");

    // Test meeting/handle endpoint to validate token
    try {
      addLog("🧪 Testing meeting/handle endpoint...", "info");

      const response = await Api({
        endpoint: "meeting/handle",
        method: METHOD_TYPE.GET,
        requireToken: false, // This endpoint uses Zoom token automatically
      });

      addLog("✅ meeting/handle Success!", "success");
      addLog(`📋 Response: ${JSON.stringify(response, null, 2)}`, "success");
    } catch (error) {
      addLog(
        `❌ meeting/handle Error: ${
          error.response?.data?.message || error.message
        }`,
        "error"
      );
...
export default memo(CreateMeetingTest);
