...
const StudentClassroomPage = () => {
  ...
  // Debug state
  console.log("🔍 STATE DEBUG - Component state:", {
    showEvaluationModal,
    selectedClassroomForEvaluation,
    showMeetingView,
  ...
  const handleClassroomEvaluation = (classroom) => {
    // Classroom API không có isRating field, nên hướng dẫn user vào meeting để đánh giá
    toast.info(
      "Vui lòng vào phòng học và đánh giá từ buổi học cụ thể để có đánh giá chính xác nhất!"
    );

    // Mở classroom meetings để user có thể chọn meeting
    handleViewMeetings(classroom.classroomId, classroom.nameOfRoom);
  };
  const handleEvaluationSubmit = async (evaluationData) => {
    console.log("Evaluation submitted:", evaluationData);
    toast.success("Đánh giá đã được gửi thành công!");

    // Cập nhật isRating = true cho meeting cụ thể trong danh sách meetings
    if (allMeetings && selectedClassroomForEvaluation?.meetingId) {
      setAllMeetings((prev) =>
        prev.map((meeting) =>
          meeting.meetingId === selectedClassroomForEvaluation.meetingId
  ...
  const handleCloseEvaluationModal = () => {
    setShowEvaluationModal(false);
    setSelectedClassroomForEvaluation(null);
  }; // Meeting rating handlers - Sử dụng ClassroomEvaluationModal thay vì MeetingRatingModal  const handleMeetingRating = (meeting) => {
    alert("🎯 Button clicked! Checking data...");
    
    const classroom = currentClassroomForMeetings;
    
    if (!meeting) {
      alert("❌ Không có meeting data");
      return;
    }

    if (!classroom) {
      alert("❌ Không có classroom data");
      return;
    }

    alert("✅ Data OK, creating modal...");
    ...
    setSelectedClassroomForEvaluation(evaluationData);
  ...
  return (
    <div className="student-classroom-page">
      ...
      {/* DEBUG: Test button để force show modal */}
      <div
        ...
        <button
          onClick={() => {
            ...
            const testData = {
              ...
              tutor: { fullname: "Test Tutor", userId: "test-tutor" },
            };
            setSelectedClassroomForEvaluation(testData);
            setShowEvaluationModal(true);
          }}
          style={{
            background: "#dc3545",
      ...
      {showEvaluationModal && selectedClassroomForEvaluation && (
        <>
          {console.log(
            "🔍 RENDER DEBUG - Rendering ClassroomEvaluationModal:",
            {
              showEvaluationModal,
              selectedClassroomForEvaluation,
            }
          ...
          <ClassroomEvaluationModal
            ...
            tutorName={
              selectedClassroomForEvaluation.tutor?.fullname ||
              selectedClassroomForEvaluation.tutorName ||
              "Gia sư"
            }
            tutorId={
              selectedClassroomForEvaluation.tutor?.userId ||
              selectedClassroomForEvaluation.tutorId
            }
            meetingId={
              selectedClassroomForEvaluation.meetingId ||
              selectedClassroomForEvaluation.selectedMeetingId
            }
            onEvaluationSubmitted={handleEvaluationSubmit}
